...
def load_jobs() -> tuple[list[dict], list[dict]]:
    """Load all job md files and split Active/Closed by the `status` field.

    Every job file lives directly in `jobs/`. Closing a job is a frontmatter
    edit (status=Closed + date_closed + closure_reason) — files are never
    moved or deleted. `jobs/closed/` is a legacy directory kept as a fallback
    for older files; any .md files there are loaded with the same status-based
    routing.
    """
    active, closed = [], []

    def _ingest(p: Path, rel_prefix: str) -> None:
        job = parse_frontmatter(p)
        job["details_file"] = f"{rel_prefix}{p.name}"
...
def _index_link(job: dict) -> str:
    """Return a relative path (from jobs/README.md) to the job's md file.

    details_file is set by load_jobs() as a repo-rooted path like
    "jobs/foo.md" or "jobs/closed/foo.md"; jobs/README.md lives in jobs/, so
    strip the leading "jobs/" to make the link relative.
    """
...
def write_index(active, closed):
    ...
    for j in active:
        rel = _index_link(j)
        lines.append(
            "| [{id}]({rel}) | {pri} | [{title}]({rel}) | {emp} | {loc} | {dl} | [{rel}]({rel}) |".format(
    ...
    for j in closed:
        rel = _index_link(j)
        lines.append(
            "| [{id}]({rel}) | [{title}]({rel}) | {emp} | {dc} | {reason} | [{rel}]({rel}) |".format(
                id=md_escape(j["job_id"]),
                rel=md_escape(rel),
                title=md_escape(j.get("title")),
                emp=md_escape(j.get("employer")),
